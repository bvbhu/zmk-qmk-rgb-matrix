# zmk-qmk-rgb-matrix 项目使用文档

> 面向 ZMK 键盘开发者，涵盖模块介绍、完整适配流程、全部配置项、键码用法、工作原理及常见问题。
...
- **50 种灯效**：单色常亮、渐变、彩虹循环、呼吸、彩带、风车、螺旋、雨滴、星光、河流、按键响应、溅射、热力图等
- **QMK 兼容布局**：`g_led_config` 格式与 QMK 完全一致，可无痛迁移
- **Flash 持久化**：设置自动保存，重启不丢失（可关闭）
...
模块已内置全部合理默认值，下方给出**完整可复制的配置模板**，包含所有可配置项，直接添加到现有配置中，按注释按需启用/修改：
...
# CONFIG_RGB_MATRIX_IS_LEFT=y           # 设定键盘是否为左半，未配置按照central为左半判断
...
CONFIG_RGB_MATRIX_EFFECT_PIXEL_FLOW=y
...
# CONFIG_RGB_WORKQ_STACK_SIZE=1024        # 独立 workqueue 栈字节（0=复用系统 workqueue，省 RAM）
...
# ---- [可选] RAM 优化 ----
# CONFIG_RGB_LED_CONFIG_CONST=y      # 将keymap.c中的led_config_t g_led_config定义改为const
# CONFIG_RGB_WORKQ_STACK_SIZE=0      # 复用系统 workqueue（如选用非0值建议640-1024）
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max)
{
    ...
    if(!led_state.num_lock) //num_lock关闭
...
// 切换指定灯效（mode 为灯效枚举值，从 1 开始按已启用灯效顺序编号）
...
把 `CONFIG_RGB_MATRIX_LED_PROCESS_LIMIT` 调小（QMK 默认 `(LED_COUNT+4)/5`，必须 `> 0`，建议 `16`/`24`/`32`）：把一帧的 LED 渲染拆成多帧完成，显著降低单帧 CPU 峰值，代价是完整画面刷新率变为 `1000 / (ceil(LED_COUNT / LIMIT) × FLUSH_LIMIT) ms`。例如 69 颗 LED + LIMIT=16 + FLUSH_LIMIT=16 → 约 12.5 完整 FPS。也可配合调大 `RGB_MATRIX_LED_FLUSH_LIMIT` 进一步节流。
